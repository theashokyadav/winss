...
void captureImage(int hwnd) {
  ...
  try {
    ...
    // final hFile = CreateFile(TEXT('captureqwsz-${DateTime.now().millisecondsSinceEpoch}.png'), GENERIC_WRITE, 0, nullptr,
    final hFile = CreateFile(TEXT('captureqwsz-${DateTime.now().year.toString()}-${DateTime.now().month.toString().padLeft(2,'0')}-${DateTime.now().day.toString().padLeft(2,'0')} ${DateTime.now().hour.toString().padLeft(2,'0')}-${DateTime.now().minute.toString().padLeft(2,'0')}.png'), GENERIC_WRITE, 0, nullptr,
...
int mainWindowProc(int hWnd, int uMsg, int wParam, int lParam) {
  switch (uMsg) {
    ...
    case WM_PAINT:
      ...
      BeginPaint(hWnd, ps);
      print("Value of hWnd: "+hWnd.toString());
      captureImage(hWnd);
      EndPaint(hWnd, ps);
